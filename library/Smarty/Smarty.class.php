...
#加载自定义插件
include_once SMARTY_PLUGINS_DIR . 'plugin.fun.php';
...
class Smarty extends Smarty_Internal_TemplateBase {
	...
	{ 
		...
		// set dirs
		$this->setTemplateDir( $confg['template_dir'] )->setCompileDir( $confg['compile_dir'] )->setPluginsDir( $confg['plugin_dir'] )->setCacheDir( $confg['cache_dir'] )->setConfigDir( $confg['config_dir'] );

		$this->left_delimiter = $confg['left_delimiter'];
		$this->right_delimiter = $confg['right_delimiter'];

		$this->force_compile = $confg['force_compile'];
		$this->compile_check = $confg['compile_check'];

		$this->caching = $confg['caching'];
		$this->cache_lifetime = $confg['cache_lifetime'];

		$plugin = array( 'function', 'block', 'compiler', 'modifier' );
		foreach ( $plugin as $v ) {
			if ( isset( $confg[$v] ) && is_array( $confg[$v] ) ) {
				foreach ( $confg[$v] as $key => $val ) {
					$this->registerPlugin( $v, $key, $val ); //默认注册函数列表
				} 
			} 
		} 
...
function smartyAutoload( $class )
{
	...
	static $_classes = array( 'smarty_config_source' => true,
